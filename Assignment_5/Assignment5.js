...
function colorCube()
{
    console.log(vertices[51])
    for(var i=1;i<stops;i++)
	    quad(0, i+1, i, 0 );
	quad(0, 1, stops, 0);
	    for(var i=1;i<stops;i++)
	    quad(stops+1, i+1, i, stops+1 );
	quad(stops+1, 1, stops, stops+1);
...
window.onload = function init() {
	...
	for(var i=0.0;i<stops;i++){
        vertices.push(vec4(Math.cos(i*2*Math.PI/stops),-1,Math.sin(i*2*Math.PI/stops),1.0));
    }
    vertices.push(vec4(0,1,0,1));
	...
	document.getElementById("material_color").onchange = function(event) {
		materialAmbient	= materialDiffuse = materialSpecular = hexToRgb( event.target.value);
	...
	//change light ambient color and intensity
		...
		document.getElementById("amb_color").onchange = function(event) {
			console.log(hexToRgb(amb_color.value));
			lightAmbient = scaleColor(hexToRgb(amb_color.value), Number(amb_inten.value));
	...
	//scale light specular percentage
		...
		document.getElementById("light_spe").onchange = function(event) {
			console.log(scaleColor(hexToRgb(light_color.value),Number(light_spe.value)));
			lightSpecular = scaleColor(hexToRgb(light_color.value), Number(light_spe.value));
